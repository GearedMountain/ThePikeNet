...
def addNote(title):
    ...
    result = db.session.execute(sql, {"title": title, "description": ""})
    db.session.commit()
...
def addTag(tagName, noteId):
    ...
    try:
        ...
        db.session.execute(sql, {"tagName": tagName.strip()})
        db.session.commit()
        ...
        result = db.session.execute(sql, {"tagName": tagName.strip()})
        ...
        if noteId:
            ...
            db.session.execute(sql, {"noteId": noteId, "tagId": tagId[0]})
